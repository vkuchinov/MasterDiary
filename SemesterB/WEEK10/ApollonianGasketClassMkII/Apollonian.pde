class Apollonian{
     ...
     Apollonian(PVector origin_, int levels_, int[] parameters_){
     ...
     float[] parseCurvatures(){
          
             float r1 = 1.0 / (float)parameters[1];
             float r2 = 1.0 / (float)parameters[2];
             float r3 = 1.0 / (float)parameters[3];
             
             println(r1, r2, r3);
             
             float ratio = 100.0 * (max(r1, r2, r3) * 1.0);
             
             println(ratio / r3);
             
             return new float[]{r1*500, r2*500, r3*500};
     ...
     void build(){
         ...
         if(level == maxLevels){
             ...
             arrayCopy(initialCirlces(parseCurvatures()), 0, circles, 1, 3);
             circles[0] = outerCircle(circles[1], circles[2], circles[3]);
      
             println("curvature of outer circle:", circles[0].curvature());
     ...
     Circle[] initialCirlces(float[] radiuses_) {
       
        Circle[] outputs = new Circle[3];
        Circle c0, c1, c2;
        float posx, posy;
      
        c0 = new Circle(radiuses_[0], new PVector(width/3, height/2));
        outputs[0] = c0;
      
        c1 = new Circle( radiuses_[1], new PVector(width/3 + radiuses_[0] + radiuses_[1], height/2));
        outputs[1] = c1;
      
        posx = (radiuses_[0]*radiuses_[0] + radiuses_[0]*radiuses_[2] + radiuses_[0]*radiuses_[1] - radiuses_[1]*radiuses_[2]) / (radiuses_[0] + radiuses_[1]);
        posy = (float)(Math.sqrt((radiuses_[0] + radiuses_[2]) * (radiuses_[0] + radiuses_[2]) - posx * posx));
      
        c2 = new Circle(radiuses_[2], new PVector(width/3 + posx, height/2 + posy));
        outputs[2] = c2;
