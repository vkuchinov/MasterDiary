class Agent{
    ...
    void draw(){
        ...
        stroke(0);
        if(this.type == 0) { strokeWeight(5); } else { stroke(255, 0, 255); strokeWeight(16); }
...
class AgentList extends ArrayList<Agent>{
    ...
    void connectAgents(Agent a_, int K_){
        ...
        if(this.size() >= K_) for(int a = 0; a < K_; a++){ if(a_ != this.get(a)) { stroke(255, 0, 255); strokeWeight(1.0); line(a_.position.x, a_.position.y, this.get(a).position.x, this.get(a).position.y); } } 
