class MultiscaleTuring{
    ...
    void next(){ 
      ...
      for(int l = 0; l < layers - 1; l++) {
        ...
        PVector minmax = new PVector(Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY);
        ...
        for (int i = 0; i < w * h; i++) { 
              ...
              minmax.x = min(minmax.x, F[i]);
              minmax.y = max(minmax.y, F[i]);
              
        }
       
        float dist = (minmax.y - minmax.x) / 2.0;
        for (int i = 0; i < w * h; i++) { F[i] = ((F[i] - minmax.x) / dist) - 1; }
