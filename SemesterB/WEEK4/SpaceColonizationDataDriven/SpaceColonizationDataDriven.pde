...
int numAttractors = 64000;
int startTrunk = 1;

float growthDistance  = 2; //2.0;
float max_attract_dist = 4;
float min_attract_dist = 2;
...
data = loadImage("portrait.jpg");

generatePoints(data);

//create root segments
for(int r = 0; r < 1; r++){
root = new Segment(new PVector(width/2 , height/2), null);
segments.add(root);
}
  ...
  if (!finished) {
    ...
    for (int si = s_len-1; si > 0; si--) {
      ...
      if (s.grow_count > 0) { //grow a new branch
        ...
        color c = data.get((int)s.position.x, (int)s.position.y);
        growthDistance = map(red(c), 0, 255, 5.9, 0.05);
...
void generatePoints(PImage data_) {
